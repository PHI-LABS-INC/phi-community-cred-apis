...
export async function GET(req: NextRequest) {
  try {
    ...
    const mint_eligibility = await verifyBalancerBalHolder(
      address as Address
    );
 ...
 */
async function verifyBalancerBalHolder(
  address: Address
): Promise<boolean> {
  try {
    const GIGABRAIN_TOKEN = "0xba100000625a3754423978a60c9317c58a424e3D";
    ...
    console.error("Error verifying BAL holdings:", error);
    throw new Error("Failed to verify BAL holdings");
  }
}
