...
const FOLLOWERS_QUERY = `
  ...
  accountGraphsStats(request: $request) {
    followers
  }
}
`;
 ...
 */
async function has100Followers(address: Address): Promise<boolean> {
  try {
    ...
    // Check each account to see if it has 100+ followers
    for (const accountItem of accountsBulk) {
      if (!accountItem.username?.ownedBy) continue;

      const followersResponse = await fetch(LENS_API_URL, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          ...
          variables: {
            request: {
              account: accountItem.username.ownedBy,
            },
          },
        }),
      });
      ...
      const followersCount =
        followersData?.data?.accountGraphsStats?.followers ?? 0;
...
export async function GET(req: NextRequest) {
  try {
    ...
    const mint_eligibility = await has100Followers(address as Address);
