...
async function verifyNounsSupporter(
  address: Address
  ...
  try {
    ...
    return [hasNounsNFTs, totalNFTs.toString()];
  } catch (error) {
...
export async function GET(req: NextRequest) {
  try {
    ...
    // Verify ownership across multiple contracts
    const result = await verifyMultipleWallets(req, verifyNounsSupporter);
    ...
    return new Response(
      JSON.stringify({
        ...
        data: "Error checking Nouns NFT ownership",
        signature: null,
      }),
      {
        status: 200,
